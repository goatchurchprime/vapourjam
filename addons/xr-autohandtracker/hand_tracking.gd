...
func _process(delta):
	...
	if handtrackingactive != lhandtrackingactive:
		...
		$VisibleHandTrackSkeleton.visible = handtrackingactive
...
func createhandskeleton():
	var axes3dscene = load("res://addons/xr-autohandtracker/axes3d.tscn")
	var stickscene = load("res://addons/xr-autohandtracker/stick.tscn")
	for j in range(OpenXRInterface.HAND_JOINT_MAX):
...
func updatehandskeleton(oxrjps):
	var xrt = xrorigin.global_transform
	for j in range(OpenXRInterface.HAND_JOINT_MAX):
		$VisibleHandTrackSkeleton.get_node("J%d" % j).global_transform.origin = xrt*oxrjps[j]
